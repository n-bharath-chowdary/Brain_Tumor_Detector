...
class Frames:
    ...
    #     current = 0
    #
    #     for i, frame in enumerate(listWF):
    #         frame.hide()
    #         if frame == self:
    #             current = i
    #
    #     target_frame = listWF[current + 1] if current < len(listWF) - 1 else listWF[current]
    #     target_frame.unhide()
    #     target_frame.readImage(jpgImg)
    #     target_frame.displayImage()
    #
    #     if current == len(listWF) - 1:
    #         self.btnView['state'] = 'disable'
    #
